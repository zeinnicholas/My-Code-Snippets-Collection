 ...
 * 
 * You can then, apply this CSS variable to any color value by using the var(--{$field_name}-color) value.
 * 
 ...
 *          --{$css_var}-color: $final_color;
 * 
 * @note: With this approach you won't need to define a default color within the field settings.
 *        This helps avoid conflicts. We will define the color here as the $default_color
...
function codesnippets_acf_dynamic_color_variable() {
    ...
    // Set a default color in HEX format. Later we will replace it with a dynamic color if found.
    $default_color = '#000';
    ...
    // Set an easy to type CSS variable name based on the field name. 
    ...
    if ( class_exists( 'ACF' ) ) {
        ...
        // Get the field object so we can use their returned array
        if ( $is_options_page ) {
     ...
     * Just uncomment the lines 76 and 85 and apply your conditional tags.
